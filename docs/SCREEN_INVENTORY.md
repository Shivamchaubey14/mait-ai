...
The same app, signed into by OTP with a store keeper's number, opens a different shell: three
tabs — **To issue**, **Store stock**, **Profile** — built 2026-09-11 from the three mockups
supplied that day. The zonal manager approves on the portal; these screens hand the stock over.

| # | Screen | Status | Contents |
| --- | --- | --- | --- |
| S1 | **To issue** | ✅ · built | Ink hero: *To issue* and the store's name in a pill; the headline is the queue ("5 waiting · 1 short") with *Approved by the zonal manager on the portal* under it. Two tiles — **Issued today** (green) and **Not collected** (yolk; tapping it lists the handovers still waiting on a Mait's code, with the code). Then one card per approved indent: `IND-13`, a pill — *Ready* green, *Short 7* amber, *Waiting 4d* red — and "Sunil Kumar · 25 Murrah". **Find an indent or a Mait** is an outlined control above the tab bar that becomes a search box. |
| S2 | **Issue** | ✅ · built | Ink hero: `IND-13 · Sunil Kumar`, "Approved 19 Aug by Zonal manager, Mathura", the way back on the right. The item card carries a stepper that opens at the most the store can hand over; yolk when the shelf is short, with the sentence that says the rest stays open on the same indent. **Flask temperature checked** — required before straws leave — holds the button until it is on. **After this issue**: store holding `18 → 0`, open on the indent `7`. One green button: **→ Issue 18 Murrah**. |
| S3 | **Issued** | ✅ · built | Green hero with the tick: "18 Murrah issued", `IND-13 · Sunil Kumar · 11:20`. The four-digit code, large and spaced, under *Read this to the Mait*. A blue notice while the app polls for the Mait to type it, turning green when they do; red with **Read out a new code** after five wrong ones. A yolk *7 Murrah still open · Part-issued* notice when the shelf was short. *Put it back* for a Mait who never collected. **Next indent** (green) and **Print slip** (outlined; opens the share sheet, and leaves the code off). |
| S4 | **Store stock** | ✅ · built | The shelf, one card per item: on hand as the figure, *set aside for a Mait* and *free to issue* beneath. **Record a delivery** opens a sheet — straws or consumables, the breed or product as chips, a stepper with 10/25/50/100 jumps, a note for the challan number. |
| S6 | **History** | ✅ · built | Every handover across the counter under the day it happened — Today, Yesterday, then dates — for today, 7 or 30 days. Each row: the time, `IND-13 · Sunil Kumar`, what went, and a pill — *Collected 11:24* green, *Waiting for code* amber, *Put back* grey. A green strip totals what was handed over per item for the window, leaving out what went back on the shelf. Searchable by Mait or indent; a row reopens the handover and its code. *Issued today* on S1 opens it on Today. |
| S5 | **Profile** | ✅ · built | Name, *Store keeper · mobile*, the store and the BMC/MCCs it serves, language, sign out. |

## Zonal manager

The third app on the same sign-in screen, built 2026-09-19. A zonal manager's job was a
browser job — watch this zone, approve its indents, chase the depot — and the job is not: they
are out at collection points and depots, and the decision a Mait is waiting on is a decision
about a yard, taken in a yard.

Five tabs, the same frame as the other two shells (`components/frame.tsx`): Ink hero welded to
the top, white cards on the grey page, one green action at the foot. One more tab than the
others carry, and it earns its place — a manager's first question is *is my zone working* and
their second is *what is waiting on me*, which are two screens. The second has a count on its
tab, so it does not have to be the one you land on. None of the Mait's shell is mounted: no
capture flow, no offline queue, because a decision queued for an hour is one taken on figures
that have moved.

| ID | Screen | Status | Contents |
| --- | --- | --- | --- |
| Z1 | **Zone** | ✅ · built | The dashboard, and the screen the app opens on. Every row in *Just happened* opens the whole record behind it (Z7) — with a chevron to say so, because without one a manager reads the feed as a ticker and never finds the record. Ink hero: *Your zone* and the zone in a pill, today's insemination count as the headline — the number somebody says out loud — with the week and month under it for the scale it sits in. Three tiles: **On yesterday** as a signed figure (green up, red down, because "2" alone does not say which way), **Maits out today**, and **Waiting on you**, which is the only tappable one because it is the only one with somewhere to go. Then **the last fortnight as bars** — every day drawn, including the empty ones, today in Ink and the best day in green, because a chart built only from busy days compresses a quiet week into a busy-looking line. A yolk line counts captures still on a handset or waiting on payment, kept out of the day's total so the zone is not overstated. **Who is working** and **where the work is** rank the week with bars relative to the leader, not to a target. **Just happened** closes it: the last eight captures as they landed — breed, village, Mait, time — which is what makes a zone read as a place with people in it rather than a total. |
| Z2 | **Indents** | ✅ · built | What every Mait has asked the zone for. Headline is the queue ("3 waiting on you · oldest 6d") with *Agreeing moves no stock — the depot hands it over* under it, because a manager who thinks this button ships straws will not understand why a Mait is still empty tomorrow. Two tiles: **Waiting on you** and **Waiting at a depot** — the second is not their work and is drawn plain to say so. A search box, then one card per request with a pill worked out from the *depot's shelf* rather than the size of the ask: *Ready* green, *Depot has 7* amber, *Depot empty* red, *No depot* plain. |
| Z3 | **The request** | ✅ · built | Ink hero: `IND-13`, the item as the title, the Mait and their code beneath, the way back on the right. A verdict card leads — the pill and the sentence that earns it ("Barsana depot has 7 of the 25. Agreeing still helps — the rest stays open for the next delivery."). Then the two figures the decision is actually taken on, side by side because they are read against each other: **They already hold** and **At the depot**. The Mait's card with their collection points as chips, their note where there is one, and a blue aside saying what agreeing actually does. **Two actions, one green**: *Approve 25 Murrah* at the foot, and above it *Reject this request* as a red link — never a second filled button. Rejecting opens a sheet that **requires a reason**, because the Mait reads the indent and not the audit log. |
| Z4 | **Stock** | ✅ · built | **By place first**, because a manager works out which way to drive before they work out who to ring. Three tiles — **Maits at zero** (red, its own figure: at zero they cannot record an event at all), **Low**, **On depot shelves** — then a switch between *Places*, *Maits* and *Depots*. A place card is one chilling centre: what is held under it, how many of its Maits are empty, the breeds, and **the depot that could fix it on the same card as the problem** — or *No depot serves this centre* where there is none, which is a different phone call. A centre with somebody at zero is washed red across the whole card with a matching border, never an edge stripe. Behind the switch, Maits emptiest first (with the other centres they cover named on the row) and depots with what is set aside and still to issue. |
| Z5 | **History** | ✅ · built | Every approval and rejection, and **where each one got to since** — *Waiting at the depot* against something approved three weeks ago is the row somebody opens their own history to find. Not a log of actions: sign-ins and reads belong on the portal's Audit log, and here they would bury the rows that matter. Two tiles, *Approved* and *Rejected*, which are also the filter — tapping a figure shows those, which is the shortest sentence for it — over a 7/30/90-day switch. Rows group under Today, Yesterday, then dates; each carries the indent, the Mait, the depot, a tick or a cross on a white chip, and on a rejection **the reason the Mait was given**, read back. |
| Z7 | **The record** | ✅ · built | W6 on a handset, opened by tapping any capture in *Just happened*. The portal puts evidence left and facts right; a phone has one column, so the order is the order somebody asks the questions in. **Four figures first** — straw (leading with the doses, because that is what the flask is short of), payment, status, the time it was recorded — then the straw number and its breed on a card of their own, where a depot slip is checked against them. **The photograph** in a dark well so it stays the brightest thing on screen, which is how it gets judged, tapping to full size on black; a record with no photo gets a light dashed frame and says so, never a dark slab that reads as a failed load. The caption says whether it came through the app's camera or out of the gallery, because six months later nobody can tell by looking. **Where it happened**: the coordinates, whose pin it is, and *Open in Maps* — a link out rather than an embedded map, since out in a village the useful thing is directions, not a picture. Then **who and what** (farmer, whether she is on the roll, collection point, Mait, animal, ear tag), **what was used** (doses and every consumable), **did it take** (the pregnancy chain), and **the audit trail** as a threaded list with the line that says it was written by the server as each step happened and never edited. Read-only throughout, with no control that looks as though it might not be. |
| Z6 | **Profile** | ✅ · built | Name, *Zonal manager · mobile* — the number is here because it is what they sign in with and the first thing anybody will ask them for when the app stops opening — their zone, the size of their patch, language, sign out. |
...
| W6 | **AI event detail** | ⬜ | Photo, GPS map, straw, payment proof, full timeline. The handset carries the same record for a zonal manager — see Z7, which is this screen's shape in one column. |
...
| W19 | **Audit log** | ✅ · built | The trail this platform has written since its first commit, finally readable outside a Django shell. **A row is a sentence, not a schema** — the API turns an action, a record and its metadata into "Completed AI event 64", and the raw metadata folds away under the row. Time with a relative reading beneath it, the actor as a disc of initials so the column can be scanned for one person, a coloured action pill, the record, the IP. Opening a row shows the metadata labelled, a real before/after diff where the call site recorded one, and the request id — which is a control: following it finds everything else recorded in the same request. **`pii_access` is what it exists for** (SRS §16) and is the only red action, with a tile and a chip of its own: who opened a farmer's Aadhaar card, who took a workbook of bank details out of the building. Filter chips are built from the trail with the count each would show, and are faceted against every filter but their own — counted off the filtered set they collapse to one chip with no way back. Read-only, and gated on its own section rather than granted with the rest of the portal. |
| W12 | **Inventory oversight** | ⬜ | Stock by Mait, low-stock exceptions. |
| W13 | **Indents** | ⬜ | Pending/stale indents, approve/reject, and the store each one waits at. |
| W14 | **Mait leaderboard** | ✅ · built | Ranked by AI count — how many inseminations each Mait actually performed — highest first, with collections and cash share beside it, because the two together are the only honest reading: a high count with a low cash share means events completed without money landing. **The range is the screen's other half.** Two date fields, always on screen and always editable, with presets (7/30/90 days, this month, last month) that *fill them in* rather than replace them — a board headed "Last 30 days" leaves the reader working out which thirty. Four tiles say what the range holds before the board says who holds it. Bars are relative to the leading Mait, since the villages differ by an order of magnitude and there is no per-Mait target. |
| W15 | **MPP coverage** | ⬜ | Members served vs. total per MPP. |
| W16 | **Exceptions** | 🟡 | Six queues of things needing a human: pending payments, failed OTPs, low stock, stale indents, overdue pregnancy checks and refused ones. Each card carries a count and a sample; each **Open** raises the queue behind it as a dialog over the page (W16b). The same six cards on W2 open the same dialog. |
| W16b | **Exception queue detail** | ✅ · built | The detail behind any W16 card, opened **over** it as a `<dialog>` rather than as a screen of its own — triage is a scan of six queues, and a link that takes an operator off the page to answer one of them costs them the other five and a click back. One dialog serves all six because the API answers them in one row shape: who or what, the identifier, the kind of thing, a state pill, the one figure, when — then a **guidance sentence**, the facts, and somewhere to go. A real modal (`showModal`), so focus trapping, Escape and inert-behind come from the browser, exactly as the photo viewer does it. **The guidance is the point.** Every one of these queues has several causes wearing one label and the cause decides who gets rung: a payment waiting on a farmer's authorisation and one waiting on a Mait's screenshot are the same row on the card and two different phone calls; an indent awaiting approval is this office's, approved-and-not-issued is the depot's, and a failed push never left this platform at all. Filter chips are those causes, and each carries the count it will show. Rows expand in place, one at a time. |
...
| W20 | **Zones** | ✅ · built | The dairy's own grouping of BMC/MCCs, and the other half of what an account can reach — Users & roles says which screens it opens, this says how much of the network it sees through them. Which is why the two sit together in the sidebar and why this screen borrows that one's access grid rather than inventing a third way to tick a list. **The chilling centres are not typed in**: there is no plant master, so the nineteen come from the distinct plants across the MPP table and the work is placing them, not naming them. Each belongs to exactly one zone, so the picker is a partition rather than a set of independent lists — a centre another zone holds is shown *disabled with the holder's name* rather than hidden, because an operator looking for NANPARA has to find out it is in Bahraich Zone, not fail to find it. Every card carries the centre's size beneath its name, since "NANPARA" decides nothing and "NANPARA — 247 MPPs, 7,449 members" does. The panel names what a save will add and remove before it happens, the way the access editor does. Whatever is in no zone yet has its own warning panel, listed by name and clickable into a new zone: until it is empty the zone rows on W2 do not add up to the tile above them, and a dashboard that quietly loses events is one nobody can reconcile. **Since 2026-09-19 the screen has a second half: the people.** A zone is a line on a map until somebody is standing inside it, and the mobile number on that person's account is now what opens the zonal manager's app — so *Who runs these zones* lists every zone-scoped account with its zone, **its number**, the size of its patch, its last sign-in and what it approved and rejected, and an **App** pill saying whether the handset opens for them at all (*App* · *Portal only* · *Zone off*, each naming a different person to fix it). The number cell is the control, as the Pages and Zones columns are on W11, and it opens a one-field editor whose hint changes as it is typed. Under it, *What they have done*: the audit trail narrowed to those accounts and written as the same sentences W19 prints, filtered by manager and window, and **by decisions rather than everything by default** — a manager who works in the portal signs in and out several times a day, and ninety sign-ins bury the two approvals somebody came to read. |
| W22 | **Super OTP** | ✅ · built | For when the SMS gateway is not delivering, at every step that sends a code: sign-in, the farmer check, online and cash payment. A **Step** pill leads each row; a farmer step names *her* and the Mait who asked, and the reveal says to call **her**, not the Mait. The asks from the app, oldest first — who (name, *Mait · code* or *Store keeper · store*), **the number on file**, when they asked and how many times, their reason, and whether their last sign-in SMS was actually delivered. **Generate code** shows the 6-digit code once, large on an Ink panel, with "Call {number on file}" and a ten-minute countdown; hiding it removes it from the page. *Somebody phoned instead* generates by number. A yellow notice leads the screen: call the number on file, never read a code to whoever rang. Refreshes every 20 seconds. |
| W21 | **Stores** | ✅ · built | Each location's store, the BMC/MCCs it serves, and the keepers who stand behind its counter. Built on the Zones screen on purpose — both draw a line around some chilling centres, and both are a partition, so a centre another store serves is shown disabled with that store's name. What a zone does not have is people: the editor lists the store's keepers and adds one from a name and the mobile number they sign in to the app with (a number a Mait already uses is refused). The table shows each store's shelf and how many approved indents are waiting at its counter; a store with no keeper is flagged, because nobody can issue there. Centres no store serves have their own panel — their Maits' indents are still issued from W13, with no code read out. |
