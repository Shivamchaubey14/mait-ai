...
def process_master_upload(self, upload_id: int) -> dict[str, int]:
    ...
    _warm_snapshot(upload)
    return result


def _warm_snapshot(upload: DataUploadLog) -> None:
    """
    Build the downloadable copy now, while nobody is waiting for it.

    The portal hands a master back as a locked, formatted workbook, and rebuilding the Member
    master measures at 165 seconds. Done on demand that is a request held open for most of
    three minutes — fine on a development machine and a proxy timeout in front of nginx. Done
    here it costs a fraction of an import that already ran for fifteen minutes, and every
    download afterwards is a file read.

    Never fatal. The import is the job; the copy is a convenience, and an admin who cannot
    download a spreadsheet is in a better position than one whose 105,000-row import is
    reported as failed because a spreadsheet could not be written. The download path rebuilds
    on demand anyway, so a failure here costs time and nothing else.
    """
    if upload.upload_type == DataUploadLog.UploadType.ASSIGNMENT:
        # Generated by this portal rather than uploaded from SAP, and it has its own
        # round-trip download. Nothing asks for a copy of it.
        return
